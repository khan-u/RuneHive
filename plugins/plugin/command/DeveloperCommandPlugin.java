...
import com.runehive.content.activity.inferno.Inferno;
...
import com.runehive.game.task.impl.ObjectPlacementEvent;
...
import com.runehive.net.packet.out.SendAnnouncement;
...
import static com.runehive.game.world.entity.combat.attack.FormulaFactory.*;
...
//                    System.out.println();
        ...
        commands.add(new Command("cammove") {
            @Override
            public void execute(Player player, CommandParser parser) {
                if (parser.hasNext(3)) {
                    int relX = parser.nextInt();
                    int relY = parser.nextInt();
                    int relZ = parser.nextInt();
                    int constantSpeed = parser.hasNext() ? parser.nextInt() : 0;
                    int variableSpeed = parser.hasNext() ? parser.nextInt() : 100;
                    
                    int localX = player.getPosition().getLocalX() + relX;
                    int localY = player.getPosition().getLocalY() + relY;
                    
                    player.send(new com.runehive.net.packet.out.SendCameraMove(localX, localY, relZ, constantSpeed, variableSpeed));
                    player.send(new SendMessage("Moving camera to relative pos: " + relX + ", " + relY + ", " + relZ, MessageColor.DEVELOPER));
                } else {
                    player.send(new SendMessage("Usage: ::cammove <relX> <relY> <relZ> [constantSpeed] [variableSpeed]", MessageColor.DEVELOPER));
                }
            }
        });

        commands.add(new Command("camturn") {
            @Override
            public void execute(Player player, CommandParser parser) {
                if (parser.hasNext(3)) {
                    int relX = parser.nextInt();
                    ...
                    player.send(new com.runehive.net.packet.out.SendCameraTurn(localX, localY, relZ, constantSpeed, variableSpeed));
                    player.send(new SendMessage("Turning camera to relative pos: " + relX + ", " + relY + ", " + relZ, MessageColor.DEVELOPER));
                } else {
                    player.send(new SendMessage("Usage: ::camturn <relX> <relY> <relZ> [constantSpeed] [variableSpeed]", MessageColor.DEVELOPER));
                }
            }
        });

        commands.add(new Command("camreset") {
            @Override
            public void execute(Player player, CommandParser parser) {
                player.send(new com.runehive.net.packet.out.SendCameraReset());
                player.send(new SendMessage("Camera reset to normal", MessageColor.DEVELOPER));
            }
        });

        commands.add(new Command("npcnanim", "npcanim") {
            @Override
            public void execute(Player player, CommandParser parser) {
                if (!parser.hasNext()) {
                    player.send(new SendMessage("Usage: ::npcnanim <animationId> [radius]", MessageColor.DEVELOPER));
                    return;
                }
                
                int animId = parser.nextInt();
                int radius = parser.hasNext() ? parser.nextInt() : 3;
                
                int count = 0;
                for (Npc npc : World.getNpcs()) {
                    if (npc != null && npc.isRegistered()) {
                        if (npc.getPosition().isWithinDistance(player.getPosition(), radius)) {
                            npc.animate(new Animation(animId));
                            count++;
                        }
                    }
                }
                
                player.send(new SendMessage("Applied animation " + animId + " to " + count + " NPCs within " + radius + " tiles", MessageColor.DEVELOPER));
            }
        });

        commands.add(new Command("npcntp", "npctp") {
            @Override
            public void execute(Player player, CommandParser parser) {
                if (!parser.hasNext(3)) {
                    player.send(new SendMessage("Usage: ::npcntp <x> <y> <z> [radius]", MessageColor.DEVELOPER));
                    return;
                }
                ...
                player.send(new SendMessage("Teleported " + count + " NPCs within " + radius + " tiles to " + targetPos, MessageColor.DEVELOPER));
            }
        });
