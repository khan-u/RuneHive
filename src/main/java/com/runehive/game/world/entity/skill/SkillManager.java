...
import com.runehive.content.skill.impl.cooking.Cooking;
import com.runehive.content.skill.impl.crafting.Crafting;
import com.runehive.content.skill.impl.farming.Farming;
import com.runehive.content.skill.impl.firemaking.Firemaking;
import com.runehive.content.skill.impl.fishing.Fishing;
import com.runehive.content.skill.impl.fletching.Fletching;
import com.runehive.content.skill.impl.herblore.Herblore;
import com.runehive.content.skill.impl.hunter.Hunter;
import com.runehive.content.skill.impl.mining.Mining;
import com.runehive.content.skill.impl.prayer.BoneSacrifice;
import com.runehive.content.skill.impl.runecrafting.Runecraft;
import com.runehive.content.skill.impl.smithing.Smelting;
import com.runehive.content.skill.impl.smithing.Smithing;
import com.runehive.content.skill.impl.thieving.Thieving;
import com.runehive.content.skill.impl.woodcutting.Woodcutting;
    ...
    private void updateSkill(Player player, int maxLevel, int newMax, Skill skill, boolean levelUp, boolean combat) {
        if (maxLevel < 99 && newMax != maxLevel) {
            ...
            if (levelUp) {
                ...
                if (newMax == 99 && !combat) {
                    World.sendMessage(
                            "<col=7B44B3>osroyale: <col=" + player.right.getColor() + ">" + player.getName() + " </col>has just reached level 99 in <col=7B44B3>" + Skill.getName(skill.getSkill()) + "</col> with a prestige of " +player.prestige.totalPrestige + "!");
