package com.runehive.game.world;
...
public final class World {
    ...
    private final TaskDeadlockWatcher taskDeadlockWatcher;

    /** The thread to watch task deadlocks on **/
    private final Thread taskDeadlockWatcherThread;
    /** The region manager. */
    ...
    private World() {
        ...
        taskDeadlockWatcher = new TaskDeadlockWatcher(taskManager);
        taskDeadlockWatcherThread = new Thread(taskDeadlockWatcher);
        taskDeadlockWatcherThread.setName("TaskDeadlockWatcher");
        taskDeadlockWatcherThread.start();
    }
    ...
    public static void save() {
        System.out.println("Saving osroyale...");
    ...
    public static void shutdown() {
        ...
        get().taskDeadlockWatcher.requestExit();
        System.exit(0);
    }
    ...
    public static void sendMessage(String... messages) {
        if (com.runehive.Config.DISABLE_BROADCASTS) return;
        for (Player player : getPlayers()) {
            if (player != null)
