package com.runehive.fs.cache.decoder;
...
public final class ObjectDefinitionDecoder implements Runnable {
    ...
    private final static Logger LOGGER = LogManager.getLogger(ObjectDefinitionDecoder.class);
    ...
    public void run() {
        ...
        // Allow dynamic object count to support different cache versions (Elvarg: 44934, osroyale: 47501)
        if (count != GameObjectDefinition.MAX_DEFINITIONS) {
            LOGGER.warn("GameObjectDefinition count mismatch: cache has " + count + ", expected " + 
                        GameObjectDefinition.MAX_DEFINITIONS + ". Using cache count.");
        }
    ...
    private static void decode(int id, ByteBuffer buffer) {
        ...
        while (true) {
            ...
            if (opcode == 1) {
                ...
                // Unknown opcode - break to prevent infinite loop (for cache compatibility with different revisions)
                LOGGER.warn("Unknown object opcode: {} (last: {}) for object id: {}. Skipping remaining opcodes for this object.", 
                            opcode, lastOpcode, id);
                break;
            }
            lastOpcode = opcode;
        }
